...
const MarkedQue = ({isOpen}) => {
  ...
  const get_userName = async (id) => {
    ...
    const [user, setUser] = useState({
      ...
      const [likedQuestions, setLikedQuestions] = useState([]);  // Liked questions === marked questions
      useEffect(() => {
        axios
      ...
      useEffect(() => {
        const fetchLikedQuestions = async () => {
          if(user[0] !== undefined){
            ...
            try {
                const response = await axios.get(`http://localhost:8081/api/questions/marked`);
